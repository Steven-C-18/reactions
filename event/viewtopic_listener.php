...
class viewtopic_listener implements EventSubscriberInterface
{
	...
	public function tpr_template_vars_before($event)
	{
		...
		$this->user_enable_reactions = !$this->auth->acl_get('u_disable_reactions') ? true : ($this->user->data['user_enable_reactions'] ? true  : false);
		...
		$this->forum_reaction_type_ids = $topic_data['forum_reaction_type_ids'];
	...
	public function tpr_user_data($event)
	{
		...
		$user_data = array_merge($user_data, [
			...
			'user_reactions_positive' 		=> $event['row']['user_reactions_positive'],
			'user_reactions_negative' 		=> $event['row']['user_reactions_negative'],
			'user_reactions_neutral' 		=> $event['row']['user_reactions_neutral'],
		]);
	...
	public function tpr_modify_post_row($event)
	{
		...
		$user_reactions_pos = !empty($event['user_poster_data']['user_reactions_positive']) ? $event['user_poster_data']['user_reactions_positive'] : 0;
		$user_reactions_neu	= !empty($event['user_poster_data']['user_reactions_neutral']) ? $event['user_poster_data']['user_reactions_neutral'] : 0;
		$user_reactions_neg = !empty($event['user_poster_data']['user_reactions_negative']) ? $event['user_poster_data']['user_reactions_negative'] : 0;
		$add = $user_reactions_pos + $user_reactions_neu + $user_reactions_neg;

		$event['post_row'] = array_merge($event['post_row'], [
			'USER_REACTIONS' 		=> $total,
			'USER_REACTIONS_POS' 	=> $this->traffic_light->percentage($user_reactions_pos, $add),
			'USER_REACTIONS_NEU'	=> $this->traffic_light->percentage($user_reactions_neu, $add),
			'USER_REACTIONS_NEG' 	=> $this->traffic_light->percentage($user_reactions_neg, $add),
		]);
		...
		$this->poster_id = $event['poster_id'];
		...
		$event['post_row'] = array_merge($event['post_row'], [
			...
			'POST_REACTIONS' 		=> $type_ids_post_row > (int) 1 ? $this->type_operator->round_counts($this->post_reactions) : (int) 0,
			...
			'S_POSTER_ADD'			=> ($user_react && $post_visible && !$user_foe && $this->auth->acl_get('u_add_reactions')) ? true : false,
