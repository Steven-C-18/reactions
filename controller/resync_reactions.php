...
class resync_reactions
{
	...
	public function resync_traffic_light($reaction_id, $u_action, $traffic_light)
	{
		$batch_size = $this->config['reactions_resync_batch'];
		...
		$sql = 'SELECT *
			FROM ' . $this->reactions_table . '
			WHERE reaction_type_id = ' . (int) $reaction_id . '
				AND reaction_traffic_light <> ' . (int) $traffic_light;
		$result = $this->db->sql_query_limit($sql, $batch_size);
		...
		if ($row = $this->db->sql_fetchrow($result))
		{
			...
			$sql = 'UPDATE ' . $this->reactions_table . '
				SET reaction_traffic_light = ' . (int) $traffic_light . '
				WHERE ' . $this->db->sql_in_set('reaction_id', $reaction_ids);
			$this->db->sql_query($sql);
			...
			if(count($poster_ids))
			{
				...
				{
					$sql = 'SELECT COUNT(reaction_traffic_light) AS count
						FROM ' . $this->reactions_table . '
					...
					$this->traffic_light->set_user_traffic_light($traffic_light, $count, $poster_id);
				}
			}
			...
			{
				$refresh_time = isset($this->config['reactions_resync_time']) ? $this->config['reactions_resync_time'] : (int) 1;
				meta_refresh($refresh_time, $u_action . "&amp;action=resync_refresh_light&amp;reaction_id=$reaction_id&amp;traffic_light=$traffic_light");
			}

			$this->template->assign_vars([
				'ICON'		=> $this->traffic_light->traffic_light_acp($traffic_light),
				'START'     => $this->language->lang('ACP_RESYNCING_REACTION'),
				'SYNC'		=> true,
				'S_REACTIONS_STYLESHEET'	=> true,
			]);
		...
		$this->traffic_light->set_reaction_type_tl_run($reaction_id);
