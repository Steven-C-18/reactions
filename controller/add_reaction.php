...
class add_reaction
{
	...
	public function __construct(
		...
		\steve\reactions\reaction\reaction_traffic_light $traffic_light,
	...
	// lets get this file finished in 0.9.5
	public function add($post_id, $type_id)
	{
		...
		$sql = 'SELECT post_id, poster_id, post_subject, topic_id, post_reaction_data, post_reactions, post_enable_reactions
			FROM ' . POSTS_TABLE . '
		...
		if (!$user_enable_reactions || !$post_row['post_enable_reactions'] || !$this->auth->acl_get('u_add_reactions') || !$this->request->is_ajax()
		...
		$add_reaction = $update_reaction = false;// old 
 		...
 		if (empty($user_row['reaction_user_id']))
		{
			...
			$this->traffic_light->adjust_counts_pos($reaction['reaction_type_traffic_light'], $post_row['poster_id'], false);
			...
			$sql_ary = [
				...
				'reaction_traffic_light'	=> (int) $reaction['reaction_type_traffic_light'],
			...
			if ($post_row['poster_id'] != $this->user->data['user_id'])
			{
				...
				$this->notification_manager->add_notifications('steve.reactions.notification.type.reaction', $notification_data);
			}
		...
		if (!empty($user_row['reaction_user_id']))
		{
			...
			$available = true;//!empty($this->config['reactions_enable_traffic_light']) ? true : false;
			if ($available)
			{
				$this->traffic_light->adjust_counts_neq($user_row['reaction_traffic_light'], $post_row['poster_id'], true);
				//debug reaction_type_tl_run
				if ($reaction['reaction_type_tl_run'])
				{
					$this->traffic_light->adjust_counts_pos($reaction['reaction_type_traffic_light'], $post_row['poster_id'], true);
				}
			}
			...
			if ($post_row['poster_id'] != $this->user->data['user_id'])
			{
				...
				$this->notification_manager->update_notifications('steve.reactions.notification.type.reaction', $notification_data);
			}
		...
		$user_data = [];
		$user_data = $this->traffic_light->get_user($post_row['poster_id']);
		$add = $user_data['user_reactions_positive'] + $user_data['user_reactions_negative'] + $user_data['user_reactions_neutral'];

		$percentage1 = $this->traffic_light->percentage($user_data['user_reactions_positive'], $add);
		$percentage2 = $this->traffic_light->percentage($user_data['user_reactions_negative'], $add);
		$percentage3 = $this->traffic_light->percentage($user_data['user_reactions_neutral'], $add);
		...
		if ($this->request->is_ajax() && ($add_reaction || $update_reaction))
		{
			...
			$data_send = [
				...
				'TYPE_DATA' 			=> $post_data,
				'USER_TOTAL'			=> $user_data['user_reactions'],
				'USER_REACTIONS'		=> $percentage1,
				'USER_REACTIONS_NEG'	=> $percentage2,
				'USER_REACTIONS_NEU'	=> $percentage3,
				'VIEW_URL'				=> $view_url,
			];
