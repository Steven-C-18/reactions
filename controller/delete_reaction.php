...
class delete_reaction
{
	...
	public function delete($post_id, $user_id)
	{
		...
		if (empty($post_id) || $post_id != $post_row['post_id'] || empty($user_id) || empty($post_row['post_id']) || empty($post_row['post_reaction_data']))
		{
			throw new \phpbb\exception\http_exception(404, 'REACTION_ERROR');
		}

		if ($post_row['reaction_user_id'] != $user_id || !$this->auth->acl_get('u_delete_reactions'))//add a_, m_
		{
			throw new \phpbb\exception\http_exception(403, 'NO_AUTH_OPERATION');
		}

		if (!check_link_hash($this->request->variable('hash', ''), 'delete_reaction') || !$this->request->is_ajax())
		{
			throw new \phpbb\exception\http_exception(403, 'NO_AUTH_OPERATION');
		}
		...
		$this->traffic_light->adjust_counts_neq($reaction['reaction_type_traffic_light'], $post_row['poster_id'], false);
		...
		$this->notification_manager->delete_notifications('steve.reactions.notification.type.reaction', $notification_data);
