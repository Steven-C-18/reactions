...
class reaction_types
{
		...
		$reaction_types_table)
	{
		...
		$this->cache_time = isset($this->config['reactions_sql_cache']) ? (int) $this->config['reactions_sql_cache'] : $this->const::TPR_CACHE_TIME;
	}
		...
		if (count($reaction_types))
		{
			...
			for ($i = 0, $end = count($reaction_types); $i < $end; ++$i)
			{
				...
				switch($switch)
				{
					case 'page':
						$vars = [
							'COUNT' 		=> $this->round_counts($type_count),
					...
					case 'admin':
						...
						$vars = [
							...
							'PERCENT'				=> $percentage,
							'REACTION_TTL'			=> $this->traffic_light->traffic_light_acp($row['reaction_type_traffic_light']),
							'RTT_LIGHT'				=> $this->traffic_light->traffic_light($row['reaction_type_traffic_light']),
	...
	public function disable_all_types()
	{
	...
	public function display_reaction_types($reaction_types, $post_id, $user_type_id, $disabled_reactions)
	{
		...
		foreach ($reaction_types as $reaction_type)
		{
			...
			$traffic_light = !empty($this->config['reactions_enable_traffic_light']) ? true : false;
			...
			$type_row = [
				...
				'RTT_LIGHT'			=> $traffic_light ? $this->traffic_light->traffic_light($reaction_type['reaction_type_traffic_light']) : '',
	...
	{
		return (string) 'gif|jpg|jpeg|png|svg|tga|tif|tiff';
	}
...
//should be in another file.
	public function tpr_common_vars($new_vars = [])
	{
		$template_vars = [
			...
			'REACTIONS_ENABLE_TRAFFIC_LIGHT'=> !empty($this->config['reactions_enable_traffic_light']) ? true : false,
			...
			'REACTIONS_SESSION'				=> $this->config['session_length'],
