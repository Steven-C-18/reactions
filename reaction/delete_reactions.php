	...
	public function update_reaction_counts($reactions)
	{
		if (!empty($reactions))
		{
			...
			{	
				...
				switch ($reaction['reaction_traffic_light']) {
					case 1:
						$sql = 'UPDATE ' . USERS_TABLE . '
							SET user_reactions_positive = user_reactions_positive - 1
							WHERE user_id = ' . (int) $reaction['poster_id'];
						$this->db->sql_query($sql);
				    break;
					case 2:
						$sql = 'UPDATE ' . USERS_TABLE . '
							SET user_reactions_neutral = user_reactions_neutral - 1
							WHERE user_id = ' . (int) $reaction['poster_id'];
						$this->db->sql_query($sql);
				    break;
					case 3:
						$sql = 'UPDATE ' . USERS_TABLE . '
							SET user_reactions_negative = user_reactions_negative - 1
							WHERE user_id = ' . (int) $reaction['poster_id'];
						$this->db->sql_query($sql);
		   			break;
				}

				$this->notification_manager->delete_notifications('steve.reactions.notification.type.reaction', $reaction['reaction_id']);
			}
			unset($reactions);
		}
		...
		if ($row = $this->db->sql_fetchrow($result))
		{
			...
			if (!empty($poster_ids))
			{
				foreach ($poster_ids as $poster_id => $count)
				{
					...
					switch ($type_data['reaction_type_traffic_light']) {
						case 1:
							$sql = 'UPDATE ' . USERS_TABLE . '
								SET user_reactions_positive = user_reactions_positive - ' . (int) $count . '
								WHERE user_id = ' . (int) $poster_id;
							$this->db->sql_query($sql);
					    break;
						case 2:
							$sql = 'UPDATE ' . USERS_TABLE . '
								SET user_reactions_neutral = user_reactions_neutral - ' . (int) $count . '
								WHERE user_id = ' . (int) $poster_id;
							$this->db->sql_query($sql);
					    break;
						case 3:
							$sql = 'UPDATE ' . USERS_TABLE . '
								SET user_reactions_negative = user_reactions_negative - ' . (int) $count . '
								WHERE user_id = ' . (int) $poster_id;
							$this->db->sql_query($sql);
			   			break;
					}
...
// needs a bit of work, move to types
				if (!empty($post_data))
				{
			...
			{
				$this->notification_manager->delete_notifications('steve.reactions.notification.type.reaction', $reaction_ids);
			}
